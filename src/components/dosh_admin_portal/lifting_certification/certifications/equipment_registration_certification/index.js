...
const EquipmentRegistrationLicenceComp = () => {
  ...
  const {
    ...
    isLoading,
  } = useSWR(`${main_url}/lifting/equipment/dosh/certificates`, fetcher);
...
<div  css={{
    ...
    { progress === "ongoing" &&
      factory?.data?.filter((item) => item.manufacturer.toLowerCase().includes(search) && item.is_approved === false).map((data, index) => (
    ...
    { progress === "completed" &&
      factory?.data?.filter((item) => item.manufacturer.toLowerCase().includes(search) && item.is_approved === true).map((data, index) => (
          <div key={index} css={ (theme) => ({
            backgroundColor:theme.colors.Primary_50,
            padding:"16px 12px",
            borderRadius:10
