...
const AmmendedDocumentUploaded = () => {
  ...
  const {
    ...
    isLoading
      ? null
      : `${main_url}/dosh/factory/ammendment/files?factory=${router.query.id}`,
    fetcher
  );
    ...
    >
      {doc_isLoading ? (
        ...
        >
          ...
          >
            {single_factory_doc.data.ammended_files.filter(
              ...
              >
                {single_factory_doc.data.ammended_files
                  .filter(
                    (word) => word.doc_type !== "ammendment_payment_reciept"
                  )
                  .map((doc) => (
...
export default AmmendedDocumentUploaded;
